...
station_maximum_precipitation = station_maximum_precipitation.reduceByKey(lambda x,y: max(x,y))
station_maximum_precipitation = station_maximum_precipitation.filter(lambda x: float(x[1])>=10 and float(x[1])<=200)
...
station_precipitation = precipitation_lines.map(lambda x: (     (x[1][0:4], x[1][5:7],x[0]),   (float(x[3]),1)   ))
station_precipitation = station_precipitation.filter(lambda x: int(x[0][0])>=1993 and int(x[0][0])<=2016)
station_sum_precipitation = station_precipitation.reduceByKey(lambda x,y:(x[0]+y[0],x[1]+y[1]))
...
ostergotland_stations_average_precipitation = station_average_precipitation.filter(lambda x: x[0][2] in ostergotland_stations )
